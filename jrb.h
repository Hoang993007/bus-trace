...
typedef struct jrb_node {
  ...
  struct jrb_node *flink;//con đầu tiên front 
  struct jrb_node *blink;//con cuối cùng back
  struct jrb_node *parent;//node cha mẹ
  Jval key;//key của node 
  Jval val;//giá trị của node
} *JRB;
//cây jrb là mở rộng của cây thông thường khi mà nút cha có nhiều hơn hai nút con, được đánh dấu với flink và blink???????????????
