/*ĐOÁN: các nút của cây JRB được thiết lập với dạng danh sách liên kết đôi. Lấy từ danh sách liên kết đôi đó để tạo nên cây JRB*/
...
/* Creates a node with key key and val val and inserts it into the tree.
   jrb_insert uses strcmp() or use "phiên bản cải tiến không phân biệt chữ hoa chữ thường" as comparison funcion.  jrb_inserti uses <>=,
...
#define jrb_traverse(ptr, JRB_tree) \
  for(ptr = jrb_first(JRB_tree); ptr != jrb_nil(JRB_tree); ptr = jrb_next(ptr))
