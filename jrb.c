...
int cmp_koPhanBietChuHoaChuThuong(char *s1,char *s2)
{
  ...
  char *s1_tmp=strdup(s1);
  char *s2_tmp=strdup(s2);

  /*đưa toàn bộ s1 và s2 về chữ thường hết để so sánh*/
  for(int i=0;i<length1;i++)
    if(('A'<=s1_tmp[i])&&(s1_tmp[i]<='Z')) s1_tmp[i]=s1_tmp[i]+32;

  for(int i=0;i<length2;i++)
    if(('A'<=s2_tmp[i])&&(s2_tmp[i]<='Z')) s2_tmp[i]=s2_tmp[i]+32;

  int cmp=strcmp(s1_tmp,s2_tmp);
