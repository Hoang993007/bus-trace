...
JRB make_jrb()//tạo 1 nút đầu tiên và sẽ bắt đầu thêm dữ liệu từ nút đầu tiên này 
{
  JRB head;
...
/*/cmp_hoathuong BEGIN *************************************************** 
int cmp_koPhanBietChuHoaChuThuong(char *s1,char *s2)
{
...
//không thể dùng phân biệt chữ hoa chữ thường một cách đơn giản như vậy được, vì ngay khi đưa vào cây, các nút cây đã được phân theo chữ cái in hoa hay in thường, điều đó rất quan trọng, vì khi duyệt cây mình sẽ không duyệt toàn bộ cây mà duyệt theo lớn đến nhỏ theo thứ tự bảng kí tự, mà kí tự in hoa thì nhỏ hơn toàn bộ kí tự in thường !!!!
//cmp_hoathuong END ******************************************/
...
//  cmp = cmp_koPhanBietChuHoaChuThuong(key, n->blink->key.s);
  ...
  while (1) {
    if (isext(n)) return n;
  //cmp = cmp_koPhanBietChuHoaChuThuong(key, getlext(n)->key.s);
